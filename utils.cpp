...
static const std::string accountKey = "DjNlDMt9LQvbYwlHA9MI5jxlpTcSwp90kYX27A3UMr+ACRBkvC+C";
...
std::stringstream DownloadStreamFromAzure(const std::string& blobname) {
   AzureBlobClient blobClient(accountName, accountKey);
...
void UploadStreamToAzure(const std::string blobname, std::stringstream& stream) {
   ...
   blobClient.setContainer("cbdp-assignment");
...
void uploadDataToAzure() {
   ...
   auto blobClient = AzureBlobClient(accountName, accountKey);
